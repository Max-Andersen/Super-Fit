...
class ProfileRepositoryImpl(network: Network) : ProfileRepository, BaseRepository() {
    private val profileApi: ProfileApi = network.getProfileApi()

    override fun getProfileLogin() = apiRequestFlow { profileApi.getProfile() }

    override fun updateBodyParams(newParams: BodyParametersDTO) =
        apiRequestFlow { profileApi.updateBodyParams(newParams) }
    ...
    override fun uploadBodyPhoto(image: MultipartBody.Part) =
        apiRequestFlow { profileApi.uploadBodyPhoto(image) }
